...
class RouterosAPI
{
     ...
     *
     * @return array                  Array with changed key names from a key-name to key_name
     */
    public function arrayChangeKeyName(&$array)
    {
        if (is_array($array)) {
            foreach ($array as $k => $v) {
                $newKeyName = str_replace("-", "_", $k);
                $newKeyName = str_replace("/", "_", $newKeyName);
                if ($newKeyName) {
                    $array[$newKeyName] = $v;
                    unset($array[$k]);
                }
     ...
     *
     * @return array                  If we are connected or not
     */
    ...
    public function write($command, $param2 = true)
    {
        if ($this->connected) {
            ...
            if (gettype($param2) == 'integer') {
                ...
                if ($param2 === true) {
                    fwrite($this->socket, $this->encodeLength(0));
                }
            }
