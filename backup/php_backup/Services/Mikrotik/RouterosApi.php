...
class RouterosAPI
{
    ...
    public function connect($ip, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            // Allow legacy cipher suites for compatibility with older RouterOS versions
            $context = stream_context_create(['ssl' => ['verify_peer' => false, 'verify_peer_name' => false, 'ciphers' => 'DEFAULT@SECLEVEL=1']]);
    ...
    public function comm($com, $arr = array())
    {
        ...
        if ($this->isIterable($arr)) {
            foreach ($arr as $k => $v) {
                // Check if $k is a string and has at least one character
                if (is_string($k) && strlen($k) > 0) {
                    ...
                    // Default case if $k is not a valid string
                    $el = "=$k=$v";
                }
